...
- `getRooms()`: return `RoomInfo[]` for the admin-only host room list (optional).
- `getRoom(roomId)`: return one `RoomInfo` or `null` for public pre-join occupancy checks (optional).
...
- Optionally provide `getRooms`, `getRoom`, and `getRoomsForRedirect` to populate admin room lists, pre-join occupancy, and the redirect modal.
