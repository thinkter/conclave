...
final class SocketIOManager: ObservableObject {
    ...
    private func emit<T: Encodable>(event: String, payload: T) async throws -> Data {
    ...
    private func jsonObject<T: Encodable>(from payload: T) throws -> SocketData {
    ...
    private func jsonData(from value: Any) -> Data? {
    ...
    private func extractError(from value: Any) -> String? {
        if let dict = value as? [String: Any],
    ...
    private func registerEventHandlers(_ socket: SocketIOClient) {
