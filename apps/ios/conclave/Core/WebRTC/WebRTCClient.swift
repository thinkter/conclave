...
final class WebRTCClient: NSObject, ObservableObject {
    ...
    @Published private(set) var localAudioEnabled: Bool = false
    @Published private(set) var localVideoEnabled: Bool = false
    ...
    private var receiveTransportId: String?
    ...
    private var consumers: [String: ConsumerInfo] = [:]

    // MARK: - RTP Capabilities (from server)

    private var serverRtpCapabilities: RtpCapabilities?
    ...
    private var rtcLocalVideoTrack: RTCVideoTrack?
    private var rtcLocalAudioTrack: RTCAudioTrack?
    ...
    private weak var socketManager: SocketIOManager?
    ...
    private func selectFPS(for format: AVCaptureDevice.Format, targetFPS: Float64) -> Float64 {
    ...
    private func setupPreviewCaptureSession() {
    ...
    private func decodeJSONString<T: Decodable>(_ string: String, as type: T.Type) throws -> T {
        let data = string.data(using: .utf8) ?? Data()
        return try JSONDecoder().decode(T.self, from: data)
    }

    private func resolveIceServersJSON() -> String? {
